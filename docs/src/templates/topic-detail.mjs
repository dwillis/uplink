...
export function renderTopicDetail(topic, topicArticles, basePath = '/') {
  const cards = [...topicArticles].reverse().map(a => articleCard(a, basePath)).join('\n');

  const content = `
<div class="page-header">
  <h1>${escapeHtml(topic)} <span class="count">(${topicArticles.length})</span></h1>
  <a href="${basePath}topics/" class="back-link">← All topics</a>
</div>
<div class="article-list">
${cards}
</div>`;

  return layout({ title: topic, content, basePath });
}
