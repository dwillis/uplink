...
export function consolidate(repoRoot) {
  ...
  // Step 2b: load slug-keyed supplements (generated by summarize_unmatched.py)
  const supplements = new Map();
  const supplementsPath = join(dataDir, 'supplements.json');
  if (existsSync(supplementsPath)) {
    const raw = readFileSync(supplementsPath, 'utf8').trim();
    if (raw) {
      for (const [slug, data] of Object.entries(JSON.parse(raw))) {
        supplements.set(slug, data);
      }
    }
  }
  ...
  for (const article of allFullText) {
    ...
    if (summary) {
      ...
      const supplement = supplements.get(slug);
      if (supplement) {
        finalSummary = supplement.summary;
        finalKeywords = supplement.keywords;
        matchedSupplement++;
      } else {
  ...
  const totalMatched = matchedExact + matchedPositional + matchedSupplement;
  console.log(`Consolidated: ${consolidated.length} articles (${totalMatched} matched: ${matchedExact} exact + ${matchedPositional} positional + ${matchedSupplement} supplement, ${unmatched} unmatched)`);
  return consolidated;
}
