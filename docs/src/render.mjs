...
export function render(articles, topicIndex, topicNames, buildDir) {
  // index.html
  write(join(buildDir, 'index.html'), renderArticleList(articles));
  ...
  for (const article of articles) {
    write(join(articleDir, `${article.slug}.html`), renderArticleDetail(article, articles));
  }
  ...
  write(join(topicsDir, 'index.html'), renderTopicList(topicIndex, topicNames));
  ...
  for (const topic of topicNames) {
    ...
    const topicArticles = topicIndex[topic]
      ...
      .filter(Boolean);
    write(join(topicsDir, `${topicSlug}.html`), renderTopicDetail(topic, topicArticles));
  }
